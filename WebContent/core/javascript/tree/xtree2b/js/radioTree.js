...
_p.getLabelHtml = function () {
	...
	if(this.radioNeed)
	{
		if(this.radioCheck)
		{
			radioValue[0] = this.radioVal;
			if(this.radioNotUse)
			{
				radioHtml = "<input type='radio' style='vertical-align: middle;' disabled checked id="+this.id+" name='treeItemRadio' value=\'"+this.radioVal+"\'/>";
			}
			else
			{
				radioHtml = "<input type='radio' style='vertical-align: middle;' checked id="+this.id+" name='treeItemRadio' value=\'"+this.radioVal+"\'/>";
			}
		}
		else
		{	
			if(this.radioNotUse)
			{
				radioHtml = "<input type='radio' style='vertical-align: middle;' disabled id="+this.id+" name='treeItemRadio' value=\'"+this.radioVal+"\'/>";
			}
			else
			{
				radioHtml = "<input type='radio' style='vertical-align: middle;' id="+this.id+" name='treeItemRadio' value=\'"+this.radioVal+"\'>";
			}
		}
	}
	
	//修改:去掉a点击后的边框线
	return radioHtml+"<a onclick=\"javascript:this.blur();\" href=\"" + webFXTreeHandler.textToHtml(this._getHref()) +
