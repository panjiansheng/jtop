...
{    
	...
	state.actState.push(command);
    }

    function peekCommand(state) { 
	if (state.actState.length>0)
	    return state.actState[state.actState.length-1];
	else
	    return null;
    }
	...
	if (state.actState.length>0) {
	    var plug = state.actState.pop();
	    plug.closeBracket();
	}	    
    }

    function applyMostPowerful(state) {
      var context = state.actState;
    ...
    return {
	 ...
	 copyState: function(s) { return { f: s.f, cmdState: s.actState.slice(0, s.actState.length) }; },
