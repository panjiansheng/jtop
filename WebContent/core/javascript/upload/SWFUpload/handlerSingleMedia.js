...
function fileQueueError(file, errorCode, message) 
{
	...
	try {
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			wd.$.dialog.tips('文件大小为0...',1);
			break;
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			wd.$.dialog.tips('文件大小超过限制...',1); 
			break;
...
function fileQueued(file)
{
	...
	var classObj = wd.document.getElementById("classId");
	

	 if(typeof(classObj) == 'undefined' || classObj == null)
	 {	 
	 	error = true;
	 	return;
	 }
	...
	//addReadyFileInfo(file.id,file.name,"成功加载到上传队列");
	 ...
	 swfu.addPostParam("classId", wd.document.getElementById("classId").value);
...
{	
	...
	{				
		 ...
		 //用于显示
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_iframe').src = basePath+'/core/content/UploadVideoModule.jsp?fileUrl='+jsonObj.obj_0.fileUrl+'&autoStart=true';
		 
		//用于显示
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_showtime').value = jsonObj.obj_0.videoT;
		 
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_width').value = jsonObj.obj_0.videoW;
		 
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_height').value = jsonObj.obj_0.videoH;
		 
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_name').value = name;
		 
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_type').value = type;
		 
		 //每次上传新的视频需要将以前截取的图片信息置空
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_cover_src').value = '';
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_cover_w').value = '';
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_cover_h').value = '';
		 wd.document.getElementById(mediaSrcId+'_sys_jtopcms_media_cover_n').value = '';
		 
		 //用于cms处理
		 wd.document.getElementById(mediaValId).value = jsonObj.obj_0.resId;
