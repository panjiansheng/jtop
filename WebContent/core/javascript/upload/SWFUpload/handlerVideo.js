...
function fileQueued(file){
	...
	swfu.addPostParam("classId", document.getElementById("classId").value);
	...
	//try {
		...
		if (this.getStats().files_queued > 0) {
			...
			for(var i = 0 ; i < serverDataArray.length; i++)
			{
				...
				//alert(jsonObj.obj_0.resizeImageUrl);
				
				//addImage(jsonObj.obj_0.resizeImageUrl);
		
		      
				var uploadImageTr = '<tr id="uploadPhotoTr-'+imageCount+'"><td width="115" >'
									 +'<img id="resizeUrl-'+imageCount+'" src="'+jsonObj.obj_0.resizeImageUrl+'" width="120" height="87" />'
									 +'<input type="hidden" id="relatePath-'+imageCount+'" name="relatePath-'+imageCount+'" value="'+jsonObj.obj_0.relatePath+'" />'
									 +'<input type="hidden" id="pefixDir-'+imageCount+'" name="pefixDir-'+imageCount+'" value="'+jsonObj.obj_0.pefixDir+'" />'
									 +'<input type="hidden" id="photoName-'+imageCount+'" name="photoName-'+imageCount+'" value="'+jsonObj.obj_0.imageName+'" />'
									 +'<input type="hidden" id="photoGenName-'+imageCount+'" name="photoGenName-'+imageCount+'" value="'+jsonObj.obj_0.genName+'" />'
									 +'<input type="hidden" id="photoOutLinkUrl-'+imageCount+'" name="photoOutLinkUrl-'+imageCount+'" value="" />'
									 +'<input type="hidden" id="orderFlag-'+imageCount+'" name="orderFlag-'+imageCount+'" value="'+imageCount+'" />'
									 +'<input type="hidden" id="height-'+imageCount+'" name="height-'+imageCount+'" value="'+jsonObj.obj_0.height+'" />'
									 +'<input type="hidden" id="width-'+imageCount+'" name="width-'+imageCount+'" value="'+jsonObj.obj_0.width+'" />'
									 +'<table width="130"  border="0" cellpadding="0" cellspacing="0">'
													+'<tr><td><img class="a_pointer" src="../../../core/style/icons/direction.png" width="16" height="16" alt="设为封面图片" onclick=\'javascript:setCoverPhoto("'+jsonObj.obj_0.resizeImageUrl+'","'+jsonObj.obj_0.relatePath+'")\'/></td>'
													+'<td><img class="a_pointer" src="../../../core/style/icons/image-crop.png" width="16" height="16" alt="裁剪图片" onclick=\'javascript:disposeImage("'+jsonObj.obj_0.imageUrl+'","'+jsonObj.obj_0.width+'","'+jsonObj.obj_0.height+'","'+jsonObj.obj_0.relatePath+'")\'/></td>'
													+'<td><img class="a_pointer" src="../../../core/style/icons/gear.png" width="16" height="16" alt="编辑图片信息"/></td>'
													+'<td><img class="a_pointer" src="../../../core/style/icons/cross-script.png" onclick=\'javascript:deletePhotoInfo('+imageCount+',"'+jsonObj.obj_0.relatePath+'")\' width="16" height="16" alt="删除图片"/></td>'
													+'</tr>'
									 +'</table>'
									 +'</td>'
									 +'<td><textarea id="photoDesc-'+imageCount+'" name="photoDesc-'+imageCount+'" style="height:90px;width:330px" class="form-textarea"></textarea>'
									 +'&nbsp;&nbsp;<img class="a_pointer" src="../../../core/style/icons/arrow-skip-090.png" onclick=\'javascript:changePhotoPos('+imageCount+',"up")\' />&nbsp;&nbsp;<img class="a_pointer" src="../../../core/style/icons/arrow-skip-270.png" onclick=\'javascript:changePhotoPos('+imageCount+',"down")\' />&nbsp;&nbsp;' 
									 +'</td></tr>';

                //alert(uploadImageTr);
				allImageInfoArray.push(jsonObj.obj_0.relatePath);
				sortArray[imageCount] = imageCount;
				
				imageCount++;
			   
				$('#imageUploadTab').append(uploadImageTr);
...
function disposeImage(imageUrl,w,h,relatePath)
{
	$.dialog({ 
       ...
       // content: 'url:'+base+'/core/content/CropImageArea.jsp?imageUrl='+imageUrl
        ...
        content: 'url:'+base+'/core/content/DisposeImage.jsp?imageUrl='+imageUrl+"&width="+w+"&height="+h+"&relatePath="+relatePath
