...
{	
	...
	if(swfu.getStats().files_queued >0)
	{
		...
		swfu.addPostParam("maxWidth", document.getElementById("maxWidth").value);//图片最大宽度
	    swfu.addPostParam("maxHeight", document.getElementById("maxHeight").value);//图片最大高度
	    swfu.addPostParam("disposeMode", document.getElementById("disposeMode").value);//缩放处理模式
 		swfu.startUpload();
	}
	else
	{
...
function uploadComplete(file) {
	//try {
		...
		if (this.getStats().files_queued > 0) {
			...
			//alert(this.getStats().files_queued + " : "+serverDataArray.length );
			...
			{				
				var jsonObj = eval("("+serverDataArray[i]+")");
				 ...
				 var imgName = jsonObj.obj_0.imageName.substring(0,jsonObj.obj_0.imageName.indexOf('.'));
		         ...
		         var newWH = checkSize(width, height, mw, mh );
		   		 ...
		   		 var postData = '?emType=2&path='+jsonObj.obj_0.relatePath+"&resId="+jsonObj.obj_0.resId+"&w="+newWH[0]+"&h="+newWH[1];
...
function uploadCompleteModule(file)
{
	...
	{				
		...
		// alert(name+":"+jsonObj.obj_0.imageUrl);
		 ...
		 wd.document.getElementById(imageId).src=jsonObj.obj_0.imageUrl;
		 
		//if(oEditor.EditMode == W.FCK_EDITMODE_WYSIWYG)
   		{   
   			//oEditor.InsertHtml("<p style='text-align: center'><img name='jtopcms_content_image' src='" + jsonObj.obj_0.imageUrl + "' alt='"+name+"' height='"+newWH[1]+"' width='"+newWH[0]+"'/>&nbsp<br><br>"+name+"</p><br><br>");          
   		}
